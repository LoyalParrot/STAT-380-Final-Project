...
Looking mostly at the table since it is easier to look through since it is arranged, it looks like `Nuketown ’84` is the most likely to be picked, followed by `Raid`, followed by `Crossroads Strike`, continue this train of thought till you reach your desired however many is the likely number you would like.
...
#### Gemini give the following plan and approach
...
The code then seems to "Parse MapVote" for trying to find voting tallys like I did. The AI gave the interpretation that: Data inspection revealed format is "WinnerVotes to LoserVotes". THIS IS WRONG. As I noted in my part, I found that the delimiter was inconsistent with WinnerVotes to LoserVotes sometimes dropping the t in 'to'. The way they got around this way by thinking that anything that didn't follow that pattern is just an invalid entry and hence should be treated like 0 to 0, which is WRONG and likely going to give some wrong proportions.
...
My results from Task 1 are (reducing rows to top ten to be consistent with Gemini):

```{r}
CandicacyMap %>% head(10) %>% kable()
...
Comparing the tables, the proportions are simply incorrect since according to the research question we need proportions to represent when they win a vote, where the Gemini table is including when it ties (which is WRONG as it is not conforming to the research question, below paragraph explains). As a consequence of that, looking at their total candidates, the numbers are larger than mine due to the fact that I removed ties as potential candidates while they did not. One thing that is similar across both our results are the number of strict wins though, take Nuketown where we both found 28 strict wins and crossroads where we both found 41 strict wins.

As a result of their candidate blunder, Gemini has a central problem with their visualization. They are considering strict wins across candidates including those that tie. To further provide an explanation of why that is wrong, we must consider the fact that the research question is asking: Which maps are the most likely to win the map vote when they are an option? To include ties, Gemini is stating that a map won the map vote, rather than being tied in the map vote. If a map is tied in a map vote then its candidacy is the result of not a win, but an arbitrary choice by the game. Hence we cannot consider those games in determining strict wins.
...
The difference that Gemini did that is better than my code is with the data cleaning that they fixed inconsistencies with what I think is something like a python dictionary. This is more efficient as for my data cleaning I was doing searches on the entire dataframe for each inconsistency. By doing it by like a python dictionary, Gemini is simply going through the dataframe only once for each entry rather than going through the dataframe each time for each inconsistency. Although Big O notation wouldn't care (O(n)), my method is notably a little more complex.
...
I have already spoke about my displeasure with how they calculate their probabilities in the python interpretation.

### Overall Results

I can't really say with confidence that the AI's solution outside of data cleaning, complexity-wise, is better than mine. Doing a comparison of processes, we both combine datasets using rbind. We both perform data cleaning manual esque and this is the only place they beat me complexity wise, (they beat me in using a space stripping command and doing cleaning in a python dictionary type manner). The Calculation part ends up being kind of the same complexity wise as well, where they use different commands but we end up with the same overall result in strict wins. With regards to amount of code, I was simply more verbose in my calculations where I showed each step. Otherwise we have pretty much the same amount of overall code lines. A notable oversight that the AI had was that they included ties in their calculations for Map Vote Wins, where ties should not be counted because if a map is tied in a map vote then its candidacy is the result of not a win, but an arbitrary choice by the game. That is the winning point for my code, otherwise Gemini's data cleaning is superior.
